...
### 6c. "Simpan ke GitHub berhasil, tapi Tarik/Sinkron Otomatis gagal"

Kalau menyimpan (push) berhasil tapi menarik (pull) — baik manual maupun
otomatis — selalu gagal, penyebabnya BUKAN token salah (karena token yang
sama sudah terbukti bekerja untuk menyimpan). Kemungkinan penyebabnya:

- **Bug versi lama**: pada versi aplikasi sebelum ini, Sinkron Otomatis
  bisa "terkunci" secara permanen setelah perubahan pertama — auto-tarik
  berhenti bekerja sama sekali sejak saat itu, walau tombolnya tetap
  terlihat aktif. Ini sudah diperbaiki; pastikan Anda memakai versi
  aplikasi terbaru (unduh ulang & unggah ulang seluruh berkas ke repo,
  lalu lakukan *hard refresh* di peramban dengan Ctrl+Shift+R /
  Cmd+Shift+R supaya tidak memakai salinan lama yang tersimpan di cache).
- **Pesan error sekarang lebih rinci**: kalau masih gagal setelah
  memperbarui, coba **Tarik dari GitHub** secara manual dan baca pesan
  errornya dengan saksama — sekarang menampilkan alasan asli dari GitHub
  (bukan lagi pesan generik), misalnya kalau ternyata soal batas API
  tercapai, repo/path yang salah, atau berkas yang ditarik ternyata bukan
