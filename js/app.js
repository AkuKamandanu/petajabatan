...
(() => {
  ...
  function scheduleAutoPush() {
    ...
    autoPushTimer = setTimeout(() => {
      autoPushTimer = null; // PENTING: reset begitu timer benar-benar tereksekusi, bukan cuma dijadwalkan.
      // Tanpa ini, checkForRemoteUpdates() akan MENGIRA masih ada push
      // tertunda selamanya setelah edit pertama (variabel ini tetap berisi
      // ID timer lama yang sudah kedaluwarsa, sehingga selalu dianggap
      // "truthy") — akibatnya auto-tarik terkunci permanen sejak saat itu.
  ...
  async function checkForRemoteUpdates() {
    ...
    try {
      ...
      lastSyncError = null; // sinkron berhasil, reset supaya error berikutnya (kalau ada) tetap dilaporkan
    } catch (err) {
      // Ditampilkan HANYA saat errornya baru/berbeda dari sebelumnya — supaya
      // kegagalan sementara yang terus berulang (mis. batas API) tidak
      // membanjiri layar dengan notifikasi setiap siklus polling, tapi
      // pengguna tetap tahu ada masalah (tidak lagi didiamkan sepenuhnya).
      console.warn('Auto-sync: gagal memeriksa pembaruan —', err.message);
      if (err.message !== lastSyncError) {
        lastSyncError = err.message;
        toast('Sinkron otomatis bermasalah: ' + err.message, 'err');
      }
    }
  }
